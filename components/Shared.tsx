...
export const Button: React.FC<ButtonProps> = ({ children, className, variant = 'primary', isLoading = false, size = 'md', ...props }) => {
    const baseClasses = "inline-flex items-center justify-center border border-transparent font-medium rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed transition-colors dark:focus:ring-offset-gray-800";
...
// --- SPINNER ---
export const Spinner = ({ className = "w-8 h-8" }: { className?: string }) => (
    <svg className={`animate-spin ${className} text-primary`} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
...
export const Badge: React.FC<BadgeProps> = ({ color, children }) => {
    ...
    // FIX: Replaced dynamic class concatenation with a static map to prevent Tailwind CSS from purging the classes in production builds.
    ...
    return (
        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${colorClasses[color]}`}>
            <svg className={`-ml-0.5 mr-1.5 h-2 w-2 ${svgColorClasses[color]}`} fill="currentColor" viewBox="0 0 8 8">
